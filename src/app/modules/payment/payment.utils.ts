...
export const initiatePayment = async (paymentData: TCustomerDetails) => {
  try {
    const response = await axios.post(config.payment_url!, {
      ...
      success_url: `${config.backend_base_url}/api/payment/confirmation?transactionId=${paymentData.tranId}`,
      fail_url: `${config.backend_base_url}/api/payment/confirmation?status=failed`,
